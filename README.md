...
- **MCP server integration** — register external Model Context Protocol servers (Streamable HTTP, legacy SSE, or stdio) and enable them per-thread. Remote servers (HTTP/SSE) support optional request headers (Bearer/API-key auth) and a connection test button. URLs are SSRF-guarded. The LLM discovers and calls their tools during streaming alongside built-in search/scrape tools. Google Drive, GitHub, Slack, etc. connect via remote MCP — not first-party OAuth.
