...
# with LLM_API_KEY unset so buildDisableReasoningParams falls back to the
# static MODEL_REASONING map (no network). Prints METRIC lines to stdout.
# Usage: sh autoresearch.sh
set -e

# Determinism: no API key => model-info fetch fails => MODEL_REASONING fallback.
unset LLM_API_KEY
