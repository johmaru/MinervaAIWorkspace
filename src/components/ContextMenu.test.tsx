...
describe("ContextMenu", () => {
  it("アイテムクリックで onClick + onClose が呼ばれる", () => {
    const onClose = vi.fn();
  ...
  it("メニュー内クリックでは onClose が呼ばれない（アイテムクリック前）", () => {
    const onClose = vi.fn();
    render(<ContextMenu x={10} y={10} items={items} onClose={onClose} />);
  ...
  it("role=menu と role=menuitem が設定される", () => {
    render(<ContextMenu x={10} y={10} items={items} onClose={vi.fn()} />);
    expect(screen.getByRole("menu")).toBeInTheDocument();
    expect(screen.getAllByRole("menuitem")).toHaveLength(3); // separator 以外
  });
});
